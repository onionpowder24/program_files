...
def plot_y_vs_estimated_y(y_actual, y_pred, save_path):
    """
    y-yプロットを作成し、画像ファイルに保存する。
    タイトルは y-y Plot(save_path) となり、save_path変数を使用します。
    ...
    y_pred : array-like
        推定値
    save_path : str
        保存先のファイルパス。タイトル部分にも埋め込みます。
    """
    plt.figure(figsize=(8, 6))
    plt.scatter(y_actual, y_pred, alpha=0.6, edgecolors='b', label='Predicted vs Actual')
    ...
    # タイトルに save_path をそのまま埋め込む
    plt.title(f"y-y Plot({save_path})")

    plt.legend()
    plt.grid(True)
    plt.savefig(save_path)
    plt.close()
    print(f"y-y プロットを保存しました: {save_path}")
